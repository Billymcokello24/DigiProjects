...
const generateCombinedPDF = async () => {
  ...
  try {
    ...
    doc.setFontSize(22)
    doc.setTextColor(colors.black[0], colors.black[1], colors.black[2])
    doc.text(`${user?.region || 'REGIONAL'} ICT PROJECTS`, pageWidth / 2, y, { align: 'center' })
    ...
    doc.text(`Active Counties: ${activeCounties}  |  Region: ${user?.region || 'Regional'}`, pageWidth / 2, y + 15, { align: 'center' })
    ...
    doc.setFontSize(8)
    doc.setTextColor(colors.darkGray[0], colors.darkGray[1], colors.darkGray[2])
    const summaryText = `This comprehensive report presents the status of all ICT Authority projects across the ${user?.region || 'Regional'} region's ${countiesReports.value.length} counties. The report consolidates submission data from county offices, detailing project phases, implementation status, and officer remarks. A total of ${totalSubs} submissions covering ${totalProj} distinct projects have been aggregated.`
    ...
    // Save - try both methods for better compatibility
    try {
      const fileName = `${user?.region || 'Regional'}_ICT_Report_${new Date().getTime()}.pdf`
      doc.save(fileName)
    } catch (e) {
      ...
      link.download = `${user?.region || 'Regional'}_ICT_Report_${new Date().getTime()}.pdf`
      link.click()
